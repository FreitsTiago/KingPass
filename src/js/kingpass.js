...
function export_pass(){
    let date = new Date(); if ((date.getMonth() + 1) < 10) {var month = '0' + (date.getMonth() + 1)} else {var month = (date.getMonth() + 1)}; if (date.getDate() < 10) {var day = '0' + date.getDate()} else {var day = date.getDate()}
    ...
    const options = {
        ...
        filters: [
          {name: 'KingPass', extensions: ['json']}
        ],
        defaultPath: `${month}-${day}-KGP`
      }
...
function delete_pass(id){
    if($('#delete_pass_pass_' + id).val() == decrypt(db.get('OLEvDbiPUr').value(),$('#delete_pass_pass_' + id).val())){
        ...
        $('#delete_pass_incorrect_' + id).html('<div class="alert alert-danger p-2 text-center" role="alert">Senha incorrreta!</div>')
    }
}
